...
GLSLTestRenderer::GLSLTestRenderer(const int w, const int h) : width(w), height(h)
{
	...
	camera = std::make_unique<Camera>(from, to, glm::vec3(0, 1, 0), 20, width / height, 1, distance(from, to));

	target = std::make_unique<RenderQuad>();


	//rayTraceProgram = createProgram("../shaders/QuadVert.glsl", "../shaders/VolPathTraceFrag.glsl");
...
void GLSLTestRenderer::render()
{
	...
	if (curSampleCount < maxSampleCount)
	{
		...
		glUniform3f(glGetUniformLocation(rayTraceProgram->getId(), "randomVector"), Random::getUnitFloat(),
		            Random::getUnitFloat(), Random::getUnitFloat());

		glUniform3fv(glGetUniformLocation(rayTraceProgram->getId(), "camera.position"), 1, value_ptr(camera->position));
		glUniform3fv(glGetUniformLocation(rayTraceProgram->getId(), "camera.u"), 1, value_ptr(camera->u));
		glUniform3fv(glGetUniformLocation(rayTraceProgram->getId(), "camera.v"), 1, value_ptr(camera->v));
		glUniform3fv(glGetUniformLocation(rayTraceProgram->getId(), "camera.w"), 1, value_ptr(camera->w));
